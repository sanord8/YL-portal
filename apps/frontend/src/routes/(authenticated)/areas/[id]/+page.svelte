    ...
    {:else if area}
      <div class="flex items-start justify-between">
        <div class="flex-1">
          ...
          {/if}
          <div class="flex flex-wrap items-center gap-4 mt-2 text-sm text-yl-gray-600">
            ...
            {#if area.bankAccount}
              <span class="inline-flex items-center gap-1.5">
                ...
                Bank: <a href="/admin/bank-accounts/{area.bankAccount.id}" class="font-medium text-blue-600 hover:text-blue-800 underline">{area.bankAccount.name}</a>
              </span>
            {:else}
              <span class="inline-flex items-center gap-1.5 text-yl-gray-500">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                </svg>
                No bank account assigned
              </span>
            {/if}
          </div>
        </div>
