<script lang="ts">
  ...
  // TanStack Query for areas - automatic caching and refetching
  const areasQuery = createQuery({
    queryKey: ['areas', 'list'],
    queryFn: () => trpc.area.listAll.query(),
  });

  // Access query data reactively
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      // Invalidate and refetch areas automatically
      await queryClient.invalidateQueries({ queryKey: ['areas'] });

      toastStore.add('Area deleted successfully', 'success');
