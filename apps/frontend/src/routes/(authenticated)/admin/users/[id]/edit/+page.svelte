<script lang="ts">
  ...
  import CreatePersonalFundModal from '$lib/components/CreatePersonalFundModal.svelte';
  ...
  // Personal fund state
  let personalFund: any = null;
  let isLoadingPersonalFund = false;
  let isCreatingPersonalFund = false;
  let showCreatePersonalFundModal = false;
  let personalFundError = '';
  ...
  async function loadPersonalFund() {
    try {
      isLoadingPersonalFund = true;
      personalFundError = '';

      // Get all departments for this user (should be 0 or 1)
      const departments = await trpc.department.list.query({ includeSpecialFunds: true });
      personalFund = departments.find((dept: any) => dept.userId === userId) || null;
    } catch (err: any) {
      console.error('Failed to load personal fund:', err);
      personalFundError = err?.message || 'Failed to load personal fund';
    } finally {
      isLoadingPersonalFund = false;
    }
  }

  async function handleCreatePersonalFund(event: CustomEvent<{
    userId: string;
    areaId: string;
    name: string;
    code: string;
    ...
    try {
      isCreatingPersonalFund = true;
      personalFundError = '';

      await trpc.department.createPersonalFund.mutate(event.detail);

      // Reload personal fund data
      await loadPersonalFund();

      // Close modal
      showCreatePersonalFundModal = false;
    } catch (err: any) {
      console.error('Failed to create personal fund:', err);
      personalFundError = err?.message || 'Failed to create personal fund';
    } finally {
      isCreatingPersonalFund = false;
    }
  }
...
<div class="max-w-2xl px-4 sm:px-0">
  ...
  {:else if originalData}
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-6">
      ...
      <!-- Personal Fund -->
      <div class="bg-white rounded-lg shadow border border-gray-200 p-4 sm:p-6">
        <h3 class="text-sm font-medium text-yl-gray-900 mb-4">Personal Fund</h3>

        <!-- Personal Fund Error -->
        {#if personalFundError}
          <div class="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
            <p class="text-xs text-red-800">{personalFundError}</p>
          </div>
        {/if}

        {#if isLoadingPersonalFund}
          <div class="text-center py-8">
            <div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-yl-green border-t-transparent mb-2" />
            <p class="text-sm text-yl-gray-600">Loading personal fund...</p>
          </div>
        {:else if personalFund}
          <!-- Display Existing Personal Fund -->
          <div class="bg-gradient-to-br from-yellow-50 to-amber-50 border-2 border-yellow-300 rounded-lg p-4">
            <div class="flex items-start justify-between mb-3">
              <div class="flex-1 min-w-0">
                <h4 class="text-base font-semibold text-yl-black">{personalFund.name}</h4>
                <p class="text-xs text-yl-gray-600">
                  {personalFund.area.name} ({personalFund.area.code}) • Code: {personalFund.code}
                </p>
              </div>
              <span class="px-2 py-1 text-xs font-medium bg-yellow-200 text-yellow-800 rounded-full flex-shrink-0">
                Personal Fund
              </span>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4">
              <div class="bg-white bg-opacity-70 rounded-lg p-3">
                <p class="text-xs text-yl-gray-600">Balance</p>
                <p class="text-base font-semibold text-yl-black mt-1">
                  {formatCurrency(personalFund.balance, personalFund.area.currency)}
                </p>
              </div>
              <div class="bg-white bg-opacity-70 rounded-lg p-3">
                <p class="text-xs text-yl-gray-600">Income</p>
                <p class="text-base font-semibold text-green-600 mt-1">
                  {formatCurrency(personalFund.income, personalFund.area.currency)}
                </p>
              </div>
              <div class="bg-white bg-opacity-70 rounded-lg p-3">
                <p class="text-xs text-yl-gray-600">Expenses</p>
                <p class="text-base font-semibold text-red-600 mt-1">
                  {formatCurrency(personalFund.expenses, personalFund.area.currency)}
                </p>
              </div>
            </div>

            {#if personalFund.description}
              <p class="text-xs text-yl-gray-600 mt-3 border-t border-yellow-200 pt-3">
                {personalFund.description}
              </p>
            {/if}
          </div>
        {:else}
          ...
          <div class="text-center py-8">
            ...
            <p class="text-sm text-yl-gray-600 mb-4">
              This user does not have a personal fund yet
            </p>
            <p class="text-xs text-yl-gray-500 mb-6 max-w-md mx-auto">
              Personal funds are private departments where users can track their own income and expenses. Movements are auto-approved.
            </p>
            <Button
              type="button"
              variant="primary"
              size="md"
              on:click={() => showCreatePersonalFundModal = true}
...
<!-- Create Personal Fund Modal -->
<CreatePersonalFundModal
  open={showCreatePersonalFundModal}
  userName={name}
  {userId}
  {availableAreas}
  isLoading={isCreatingPersonalFund}
  on:confirm={handleCreatePersonalFund}
  on:cancel={() => showCreatePersonalFundModal = false}
/>
