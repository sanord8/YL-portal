<script lang="ts">
  ...
  import CategorizationHistory from '$lib/components/CategorizationHistory.svelte';
  ...
  {:else if movement}
    ...
    <div class="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
      ...
      <div class="p-6 space-y-6">
        ...
        {#if movement.sourceBankAccount || movement.destinationBankAccount}
          <div>
            <h3 class="text-lg font-semibold text-yl-black mb-4">Bank Account</h3>
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {#if movement.sourceBankAccount}
                <div>
                  <dt class="text-sm font-medium text-yl-gray-600">Source Bank Account</dt>
                  <dd class="mt-1">
              ...
              {/if}
              {#if movement.destinationBankAccount}
                <div>
                  <dt class="text-sm font-medium text-yl-gray-600">Destination Bank Account</dt>
                  <dd class="mt-1">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                      <svg class="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                      </svg>
                      {movement.destinationBankAccount.name}
                    </span>
                  </dd>
                </div>
              {/if}
              {#if movement.isInternalTransfer}
                <div class="sm:col-span-2">
                  <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-start">
                    <svg class="w-5 h-5 text-gray-600 mt-0.5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <div>
                      <p class="text-sm font-medium text-gray-800">Internal Transfer</p>
                      <p class="text-xs text-gray-600 mt-1">This movement is between areas in the same bank account and does not count toward income/expense totals.</p>
                    </div>
                  </div>
                </div>
              {/if}
        ...
        <CategorizationHistory {movementId} />
        ...
        <div class="space-y-6">
          <!-- File Upload -->
          <div>
            <h3 class="text-lg font-semibold text-yl-black mb-4">Upload Attachments</h3>
            <FileUpload
        ...
        {#if (movement.parent || (movement.children && movement.children.length > 0))}
          <div>
            <h3 class="text-lg font-semibold text-yl-black mb-4">
            ...
            {#if movement.parent}
              <div class="bg-purple-50 border-l-4 border-purple-400 p-4 mb-4">
                <div class="flex items-start">
                  <svg class="w-5 h-5 text-purple-600 mt-0.5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5m0 0l5 5m-5-5v12" />
                  </svg>
                  <div class="flex-1">
                    <p class="text-sm font-medium text-purple-900 mb-1">Part of Split Movement</p>
                    <div class="bg-white border border-purple-200 rounded-lg p-3">
                      <p class="text-sm font-medium text-yl-black">{movement.parent.description}</p>
                      <p class="text-xs text-yl-gray-600 mt-1">
                        Total amount: {formatCurrency(movement.parent.amount)}
                      </p>
                      <a
                        href="/movements/{movement.parent.id}"
                        class="text-xs text-purple-600 hover:text-purple-800 font-medium mt-2 inline-block"
                      >
                        View parent movement →
                      </a>
                    </div>
                  </div>
                </div>
                ...
                <div class="flex items-start">
                  ...
                  <div class="flex-1">
                    <p class="text-sm font-medium text-purple-900 mb-3">
                      This movement has been split into {movement.children.length} parts:
                    </p>
                    <div class="space-y-2">
                      {#each movement.children as child}
                        <a
                          href="/movements/{child.id}"
                          class="block bg-white border border-purple-200 rounded-lg p-3 hover:border-purple-400 hover:shadow-sm transition-all"
                        >
                          <div class="flex justify-between items-start">
                            <div class="flex-1">
                              <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="text-sm font-medium text-yl-black">{child.area?.name || 'Unknown Area'}</span>
                              </div>
                              <p class="text-xs text-yl-gray-600 pl-6">
                                {child.area?.code || 'N/A'}
                                {#if child.department}
                              ...
                              {#if child.description && child.description !== movement.description}
                                <p class="text-xs text-yl-gray-700 mt-1 pl-6">{child.description}</p>
                              {/if}
                            </div>
                            <p class="text-sm font-bold text-purple-700 ml-4">
